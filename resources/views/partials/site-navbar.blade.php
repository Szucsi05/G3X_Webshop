...
<nav class="navbar">
    <div class="navbar-top">
        ...
        <div class="nav-right">
            <a href="#" class="nav-btn nav-btn-inline" onclick="toggleSidebar()"><img src="{{ asset('icons/category.png') }}" alt="Categories" class="icon-18"> Categories</a>
            @auth
                <div class="user-menu-container">
                    <button class="user-btn"><img src="{{ asset('icons/login.png') }}" alt="Account" class="icon-18"> {{ Auth::user()->name }}</button>
            ...
            @endauth
            <a href="{{ route('cart.index') }}" class="nav-btn nav-btn-inline" @if($cartLinkOnclick) onclick="{{ $cartLinkOnclick }}" @endif><img src="{{ asset('icons/cart.png') }}" alt="Cart" class="icon-18"> Cart <span id="cart-badge" class="badge-alert {{ $cartBadgeClass }}">{{ $cartCount }}</span></a>
        </div>
    </div>
</nav>